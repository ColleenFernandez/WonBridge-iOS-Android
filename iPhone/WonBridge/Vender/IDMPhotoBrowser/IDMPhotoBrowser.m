...
- (void)viewDidLoad {
    ...
    // Counter Label
    CGFloat indicatorWidth = 95;
    _indicatorLabel = [[UILabel alloc] initWithFrame:CGRectMake((self.view.frame.size.width - indicatorWidth)/2, 30, 95, 40)];
    _indicatorLabel.textAlignment = NSTextAlignmentCenter;
    _indicatorLabel.backgroundColor = [UIColor clearColor];
    _indicatorLabel.font = [UIFont fontWithName:@"Helvetica" size:17];
    
    if(_useWhiteBackgroundColor == NO) {
        _indicatorLabel.textColor = [UIColor whiteColor];
        _indicatorLabel.shadowColor = [UIColor darkTextColor];
        _indicatorLabel.shadowOffset = CGSizeMake(0, 1);
    }
    else {
...
//	_nextButton.enabled = (_currentPageIndex < [self numberOfPhotos]-1);
    ...
    if ([self numberOfPhotos] > 1) {
        _indicatorLabel.text = [NSString stringWithFormat:@"%lu %@ %lu", (unsigned long)(_currentPageIndex+1), IDMPhotoBrowserLocalizedStrings(@"of"), (unsigned long)[self numberOfPhotos]];
    } else {
