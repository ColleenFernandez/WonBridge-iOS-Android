...
class WBTimeLineTextParser: NSObject {
    
    class func parseText(text: String, font: UIFont, color: UIColor) -> NSMutableAttributedString? {
    ...
    private class func enumerateURLParser(attributedText: NSMutableAttributedString) {
        let URLsResults = WBChatTextParseHelper.regexURLs.matchesInString(
        ...
        for URL: NSTextCheckingResult in URLsResults {
            ...
            let highlightBorder = WBChatTextParseHelper.highlightBorder
            if (attributedText.yy_attribute(YYTextHighlightAttributeName, atIndex: UInt(URL.range.location)) == nil) {
                attributedText.yy_setColor(UIColor(netHex: 0x5B5B5B), range: URL.range)
                attributedText.yy_setUnderlineColor(UIColor(netHex: 0x5B5B5B), range: URL.range)
                attributedText.yy_setUnderlineStyle(.StyleSingle, range: URL.range)
...
class WBChatTextParser: NSObject {
    
    class func parseTextNoParser(text: String, font: UIFont, color: UIColor) -> NSMutableAttributedString? {
...
//        self.enumerateEmotionParser(attributedText, fontSize: font.pointSize)
    ...
    private class func enumeratePhoneParser(attributedText: NSMutableAttributedString) {
        let phonesResults = WBChatTextParseHelper.regexPhoneNumber.matchesInString(
            attributedText.string,
            options: [.ReportProgress],
            range: attributedText.yy_rangeOfAll()
        )
        for phone: NSTextCheckingResult in phonesResults {
            ...
            if (attributedText.yy_attribute(YYTextHighlightAttributeName, atIndex: UInt(phone.range.location)) == nil) {
                attributedText.yy_setColor(UIColor(netHex: 0x1F79FD), range: phone.range)
                let highlight = YYTextHighlight()
                highlight.setBackgroundBorder(highlightBorder)
    ...
    private class func enumerateURLParser(attributedText: NSMutableAttributedString) {
        ...
        for URL: NSTextCheckingResult in URLsResults {
            ...
            let highlightBorder = WBChatTextParseHelper.highlightBorder
            if (attributedText.yy_attribute(YYTextHighlightAttributeName, atIndex: UInt(URL.range.location)) == nil) {
                attributedText.yy_setColor(UIColor(netHex: 0xfdc0c0), range: URL.range)
                let highlight = YYTextHighlight()
                highlight.setBackgroundBorder(highlightBorder)
...
//        let emoticonResults = WBChatTextParseHelper.regexEmotions.matchesInString(
//            attributedText.string,
//            options: [.ReportProgress],
//            range: attributedText.yy_rangeOfAll()
//        )
