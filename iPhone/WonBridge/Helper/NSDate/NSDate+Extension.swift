...
func NSDateTimeAgoLocalizedStrings(key: String) -> String {
    ...
    NSUserDefaults.standardUserDefaults().setObject(["zh-Hans", "en"], forKey: "AppleLanguages")
    NSUserDefaults.standardUserDefaults().synchronize()
    
//    print(NSLocalizedString(key, tableName: "NSDateTimeAgo", bundle: bundle, comment: ""))
...
extension NSDate {
    ...
    public var millisecondesInt: Int64 {
...
//        print(calander.components([.Second, .Minute, .Hour, .Day, .Month, .Year], fromDate: self, toDate: NSDate(), options: []))
    ...
    class func utcString() -> String {
        
        let calendar = NSCalendar.currentCalendar()
        calendar.timeZone = NSTimeZone(abbreviation: "UTC")!
        let dateComponent = calendar.components([.Second, .Minute, .Hour, .Day, .Month, .Year], fromDate: NSDate())
        
        return String(format: "%d%02d%02d,%02d:%02d:%02d", dateComponent.year, dateComponent.month, dateComponent.day, dateComponent.hour, dateComponent.minute, dateComponent.second)
    }
